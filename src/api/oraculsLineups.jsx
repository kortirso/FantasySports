...
const fetchOraculsLineup = async (oraculId, weekId, accessToken) => {
  try {
    ...
    const response = await fetch(`${API_HOST}/api/v1/oraculs/lineup?oracul_id=${oraculId}${weekId ? `&week_id=${weekId}` : ''}&response_include_fields=${responseIncludeFields}&api_access_token=${accessToken}`, {
      method: "GET"
    });
