...
const refreshCache = async (cacheKey, expirationTime, fetchFunction) => {
  ...
  const jsonValue = JSON.stringify({ data: result, expires_at: Date.now() + expirationTime });
  await AsyncStorage.setItem(cacheKey, jsonValue);

  return result;
};

const fetchFromCache = async (useCache, cacheKey, expirationTime, fetchFunction) => {
  // if no using cache
  if (!useCache) return refreshCache(cacheKey, expirationTime, fetchFunction);
  ...
  if (parsedJsonValue && parsedJsonValue.expires_at > Date.now()) return parsedJsonValue.data;
