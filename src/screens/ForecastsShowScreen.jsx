...
export default function ForecastsShowScreen({navigation, route}) {
  ...
  const toggleGameDay = key => {
    setPageState({
      ...pageState,
      collapseData: {
        ...pageState.collapseData,
        [key]: !pageState.collapseData[key],
      },
    });
  };
  ...
  const groupGames = gamesData => {
    return gamesData.reduce((result, game) => {
      const convertedTime = game.start_at
        ? convertDate(game.start_at)
        : 'unknown';

      if (result[convertedTime] === undefined) {
        result[convertedTime] = [game];
      } else {
        result[convertedTime].push(game);
      }
  ...
  const fetchPageData = async ({cache}) => {
    const fetchLineup = async () =>
      await fetchOraculsLineup(oraculId, currentWeekId, authState.accessToken);
    const fetchRound = async (useCache, id) =>
      await fetchFromCache(
        useCache,
        `cupsRound-${id}`,
        MILLISECONDS_IN_DAY,
        () => fetchCupsRound(id, authState.accessToken),
      );
    const fetchPairs = async cupsRoundId =>
      await fetchAllCupsPairs(cupsRoundId, authState.accessToken);
    const fetchSeasonWeek = async (useCache, id) =>
      await fetchFromCache(useCache, `week-${id}`, MILLISECONDS_IN_DAY, () =>
        fetchWeek(id, authState.accessToken),
      );
    const fetchGames = async weekId =>
      await fetchAllGames(weekId, authState.accessToken);
    ...
    try {
      ...
      if (oraculsLineupData.periodable_type === 'Cups::Round') {
        Promise.all([
          fetchRound(cache, oraculsLineupData.periodable_id),
          fetchPairs(oraculsLineupData.periodable_id),
          ...
          setPageState({
            ...
            forecasts: oraculsLineupData.forecasts.data.map(
        ...
        Promise.all([
          fetchSeasonWeek(cache, oraculsLineupData.periodable_id),
          fetchGames(oraculsLineupData.periodable_id),
  ...
  }, [currentWeekId, oraculId]); // eslint-disable-line react-hooks/exhaustive-deps

  const renderLoadingScreen = () => <View style={styles.contentContainer} />;
  ...
  const renderForecastables = forecastables =>
    forecastables.map((forecastable, index) => (
      <Forecastable
        ...
        forecast={pageState.forecasts.find(
          element => element.forecastable_id === forecastable.id,
        )}
    ...
    return (
      <View style={styles.contentContainer}>
        <View style={{marginBottom: 12, alignSelf: 'flex-start'}}>
          <Badge
            value={
              pageState.periodableType === 'Week'
                ? `${strings.forecasts.week} ${pageState.periodable.position}`
                : pageState.periodable.name
            }
          />
        </View>
        <View style={{marginBottom: 12}}>
          <Text style={{fontSize: 24}}>
            {pageState.periodableType === 'Week'
              ? strings.forecasts.weekResults
              : strings.forecasts.roundResults}
          </Text>
        </View>
        <View style={{marginBottom: 12}}>
        ...
        <View style={styles.pointsBox}>
          <View
            style={[
              styles.pointBox,
              {borderRightWidth: 1, borderRightColor: Colors.stone300},
            ]}>
          ...
          <View
            ...
            <Text style={styles.pointBoxTitle}>
              {strings.forecasts.avgPoints}
            </Text>
            <Text style={styles.pointBoxValue}>
              {pageState.lineupsData.avg}
            </Text>
          </View>
          <View style={styles.pointBox}>
            <Text style={styles.pointBoxTitle}>
              {strings.forecasts.maxPoints}
            </Text>
            <Text style={styles.pointBoxValue}>
              {pageState.lineupsData.max}
            </Text>
          </View>
        </View>
          ...
          <View style={{flex: 1}}>
            {pageState.periodable.previous_id ? (
              <Button
                ...
                onPress={() =>
                  navigation.navigate('ForecastShow', {
                    oraculId: oraculId,
                    currentWeekId: pageState.periodable.previous_id,
                  })
                }
              />
            ) : null}
            ...
            {pageState.periodable.next_id ? (
              <Button
                ...
                onPress={() =>
                  navigation.navigate('ForecastShow', {
                    oraculId: oraculId,
                    currentWeekId: pageState.periodable.next_id,
                  })
                }
              />
            ) : null}
        ...
        {Object.entries(pageState.forecastables).map(
          ([key, forecastables], index) => (
            <View
              style={[
                index === Object.keys(pageState.forecastables).length - 1
                  ? {}
                  : {marginBottom: 12},
              ]}
              key={key}>
              <Pressable
                style={styles.pressableDay}
                onPress={() => toggleGameDay(key)}>
                <Text style={styles.pressableDayValue}>{key}</Text>
              </Pressable>
              {pageState.collapseData[key]
                ? null
                : renderForecastables(forecastables)}
            </View>
          ),
        )}
      </View>
